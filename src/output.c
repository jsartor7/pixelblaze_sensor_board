...
void updateNoteMags(uint16_t * magnitude, int downsampleFreq, int minFreq, int maxFreq, int scaleShift, uint16_t numSteps, uint16_t * noteMags) {
	...
	for (int i = 1; i < numSteps/2; i++)
	{
		...
		if (currFreq > FIX16_FROM_INT(minFreq) && currFreq < FIX16_FROM_INT(maxFreq))
		{
			...
			if( distance < FIX16_DIV(stepSize,3) && distance < FIX16_DIV(reject_distance, 3)){
				...
				if (currFreq > FIX16_FROM_INT(4200))
				{
					noteMags[noteNum] += 0;
				}
				...
				//else if (currFreq > FIX16_FROM_INT(500))
				//{
				//    noteMags[noteNum] += magnitude[i] << 1;
				//}
				else
				{
					noteMags[noteNum] += (magnitude[i] >> 2) << scaleShift;
				}
			}
		}
	}
...
void processSensorData(int16_t * audioBuffer, int16_t * audioMidHzBuffer, volatile uint16_t adcBuffer[7], volatile int16_t accelerometer[3]) {
	...
	//updateNoteMags(&magnitude[0], 20000 / LOW_N_DOWNSAMPLE,s LOW_N, &noteMags[0]);
